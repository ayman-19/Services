	...
	public sealed class UserRepository : Repository<User>, IUserRepository
	{
		...
		public async Task<bool> EmailOrPhoneIsExist(string emailOrPhone)
		{
			...
			else
			{
				...
				return validPhoneNumber ? await _context.Set<User>().AnyAsync(email => email.Phone == emailOrPhone) : false;
			}
		}

		public async Task<User> GetByIdAsync(Guid id) => await _context.Set<User>().AsNoTracking().FirstAsync(user => user.Id == id);

		public async Task<User> GetByEmailAsync(string email) => await _context.Set<User>().AsTracking().FirstAsync(user => user.Email == email);

		public async Task<User> GetByPhoneAsync(string phone) => await _context.Set<User>().AsTracking().FirstAsync(user => user.Phone == phone);
	}
}
