...
namespace Services.Persistence.Repositories
{
	public sealed class JWTManager : IJWTManager
	{
		...
		public Task<string> GenerateCodeAsync() => Task.FromResult(new Random().Next(1, 100000).ToString("D6"));
		...
		public async Task<User> LoginAsync(User user, CancellationToken cancellationToken = default)
		{
			...
			if(user.Token != null)
			{
				...
				{
					await _context.Set<Token>().Where(t => t.UserId == user.Id).ExecuteDeleteAsync();
